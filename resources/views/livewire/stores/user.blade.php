                ...
                <div class="flex justify-center mt-6">
                    <div class="w-full sm:w-auto">
                        ...
                        @if (auth()->user()->stores->count() > 0)
                            ...
                            <p class="text-center text-sm text-gray-500 dark:text-gray-400 mt-2">Puedes crear {{ 1 - auth()->user()->stores->count() }} tienda más</p>
                        ...
                        @else
                            <a href="{{ route('stores.create') }}" class="inline-flex items-center justify-center w-full px-3 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                Crear Tienda
                            </a>
                            {{-- mostrar el conteo de tiendas permitidas por crear --}}
                            <p class="text-center text-sm text-gray-500 dark:text-gray-400 mt-2">Puedes crear {{ 1 - auth()->user()->stores->count() }} tienda más</p>
                        @endif
                    </div>
                </div>
